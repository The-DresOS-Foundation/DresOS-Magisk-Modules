#!/system/bin/sh
##############################################################################
#  DresOS WebView: post-fs-data.sh   (bootloop sentinel, runs before zygote)
#
#  If a boot_pending marker survived from the previous boot, that boot crashed
#  before service.sh finished, so disable the module and mark it inert. Else
#  drop a fresh boot_pending marker for service.sh to clear on success. Never
#  touches the WebView selection; all activation is deferred to service.sh.
##############################################################################
